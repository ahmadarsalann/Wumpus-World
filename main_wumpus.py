...
if do_suite:
    ...
    mean_list = []
    for (score, num_steps) in results:
        mean_list.append(score)
    print("The mean is :")
    print(sum(mean_list) / len(mean_list))
    # WumpusEnvironment.moved_alot = sorted(WumpusEnvironment.moved_alot.items(), key=lambda x: x[1], reverse=True)

    # print(WumpusEnvironment.moved_alot)
